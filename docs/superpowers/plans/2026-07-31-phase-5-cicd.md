...
**Architecture:** One Entra ID app registration with three federated credentials (main branch, pull requests, `lab` environment) and Contributor on the subscription. GitHub holds only identifiers (client/tenant/subscription IDs) plus lab config values as secrets; Terraform authenticates natively via `ARM_USE_OIDC` + `ARM_CLIENT_ID`/`ARM_TENANT_ID` env (a gap in the current workflows — they set `ARM_USE_OIDC` but never hand the provider a client ID). The drift workflow is rewritten to use the Phase 4 CLI's exit-code contract (0/2/1) against the committed `desired-records.json` snapshot (ADR-006), gated on whether the lab currently exists. A second plan/apply job pair covers the `terraform/cloudflare` stack.
...
*Pure file edits; parallelizable with Tasks 1–3. Three defects to fix: (a) Terraform steps set `ARM_USE_OIDC` but never `ARM_CLIENT_ID`/`ARM_TENANT_ID`, so provider+backend auth would fail even with `azure/login` present; (b) `drift.yml` calls a nonexistent `ddi-reconciler/cli.py` with ad-hoc pip installs and treats every failure as drift; (c) the Cloudflare stack has no CI at all.*
