...
| Test-VM proof | BLOCKED — original apply created two NICs and the app VM, but regional core quota blocked the management VM; the explicitly approved 4-to-6 Total Regional Cores request failed with `ResourceNotAvailableForOffer`, leaving the effective limit at 4; the old recovery plan SHA-256 `cda9d41188a4c3cb7920208a8fefbf77349d003e3672181cf463afb1325faf35` is superseded and must never be applied |
| Destroy/recreate proof | DEFERRED — base lab is deployed; no destroy plan has been approved or applied |

## Final verification statement

The Phase 2 plan remains security/cost gated and blocked at Checkpoint C. Checkpoint A and the 32-resource base stack remain verified. The explicitly approved Checkpoint C plan partially applied: both test NICs and the app VM succeeded, but Total Regional Cores reached 4 of 4 and Azure rejected the management VM. App-side route, DNS, and registration proofs passed; egress diagnosed a missing explicit Standard-public-IP NVA transit allow. The user approved a 4-to-6 regional-core request and conditional recovery apply, but Azure returned terminal `Failed` state with `ResourceNotAvailableForOffer`; the effective limit remains 4 of 4, so the plan was not applied and no VM was started. Both existing VMs were subsequently verified deallocated, with 35 Terraform resources tracked, no resolver, and no destroy action. The pre-review recovery plan `checkpoint-c-recovery-regional-cores-nva-nsg.tfplan` and SHA-256 `cda9d41188a4c3cb7920208a8fefbf77349d003e3672181cf463afb1325faf35` are retained only as historical evidence and are **superseded**. Any future apply requires a fresh state-backed saved plan from current `HEAD`, full delta review, a new hash approval, and the applicable capacity gate. Sanitized evidence is in `docs/evidence/phase2/checkpoint-c-recovery.md`.
