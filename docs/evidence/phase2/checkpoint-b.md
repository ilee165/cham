...
North Central US `standardBasv2Family` usage is 2 of 4 vCPUs after the base apply. Two simultaneous `Standard_B2ats_v2` verification VMs would require four additional family vCPUs and cannot be applied under the current limit. The explicitly approved request to raise the limit to 6 was rejected by Azure as `ResourceNotAvailableForOffer`; details and the smallest viable alternative are recorded in `checkpoint-c-capacity.md`. At the time this Checkpoint B evidence was captured, no Checkpoint C plan had been generated or applied; current status is recorded in `checkpoint-c-recovery.md`.
